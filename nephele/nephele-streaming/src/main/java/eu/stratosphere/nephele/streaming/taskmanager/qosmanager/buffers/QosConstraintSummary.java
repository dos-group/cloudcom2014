...
public class QosConstraintSummary implements IOReadableWritable {
	...
	private void createMemberStatsArray(int sequenceLength, boolean sequenceStartsWithVertex) {
		...
		for(int i=0; i< sequenceLength; i++) {
			if (nextIsVertex) {
				this.aggregatedMemberStats[i] = new double[] { 0, 0, 0 };
			} else {
				this.aggregatedMemberStats[i] = new double[] { 0, 0 };
			}
			nextIsVertex =! nextIsVertex;
		}
	}

	public void addMemberSequenceSummary(QosSequenceSummary sequenceSummary) {
		if (this.isFinalized) {
			throw new RuntimeException(
					"Cannot add sequence to already finalized summary. This is a bug.");
		}

		double[][] memberStats = sequenceSummary.getMemberStatistics();
	...
	public void mergeOtherSummary(QosConstraintSummary constraintSummary) {
		...
		double[][] memberStats = constraintSummary.getAvgSequenceMemberLatencies();
		for (int i = 0; i < memberStats.length; i++) {
			for (int j = 0; j < memberStats[i].length; j++) {
				// apply a weighting proportional to noOfSummarizedSequences
				this.aggregatedMemberStats[i][j] += noOfSummarizedSequences * memberStats[i][j];
			}
		}
		
		this.aggregatedTotalLatency += noOfSummarizedSequences * constraintSummary.getAvgSequenceLatency();
		this.noOfSequences += noOfSummarizedSequences;
		this.noOfSequencesAboveConstraint += constraintSummary.getNoOfSequencesAboveConstraint();
		this.noOfSequencesBelowConstraint += constraintSummary.getNoOfSequencesBelowConstraint();
